...
begin
  ...
  if v_api is null or nullif(v_api->>'content','') is null then
    raise exception 'Public property GitHub payload is invalid';
  end if;
  ...
  select
    item->>'title' title,
    nullif(item->>'description','') description,
    case when nullif(item->>'price','')::numeric >= 10000 then nullif(item->>'price','')::numeric else null::numeric end price,
    nullif(item->>'location_city','') location_city,
    nullif(item->>'location_state','') location_state,
    nullif(item->>'location_address','') location_address,
    nullif(item->>'property_type','') property_type,
    nullif(item->>'bedrooms','')::numeric::integer bedrooms,
    nullif(item->>'bathrooms','')::numeric::integer bathrooms,
    ...
    0.80::numeric anti_fraud_score,
    coalesce((item->>'is_verified')::boolean,false) is_verified,
    ...
    'market'::text listing_market,
    false is_auction,
    null::text sale_mode,
    nullif(item->>'source_property_id','') source_property_id,
    nullif(item->>'contact_name','') contact_name,
    nullif(item->>'contact_phone','') contact_phone,
    nullif(item->>'contact_whatsapp','') contact_whatsapp,
    nullif(item->>'contact_email','') contact_email,
    now() last_seen_at
  ...
  select title,description,price,location_city,location_state,location_address,property_type,
    bedrooms,bathrooms,area_sqm,images,source_url,source_portal,anti_fraud_score,is_verified,
    scanned_at,metadata,listing_market,is_auction,sale_mode,source_property_id,contact_name,
  ...
  on conflict(source_url) do update set
    ...
    images=case when coalesce(array_length(excluded.images,1),0)>0 then excluded.images else property_search_index.images end,
    ...
    listing_market='market', is_auction=false,
  ...
  where target.listing_market='market'
    ...
    and not exists(select 1 from tmp_public_property_refresh staged where staged.source_url=target.source_url);
