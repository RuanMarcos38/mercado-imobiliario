...
async function osmNearbyCounts(coordinates: { lat: number; lng: number }, radius = 2200) {
  ...
  for (const raw of elements) {
    ...
    if (tags["public_transport"] || String(tags["highway"] ?? "") === "bus_stop")
      counts.transit += 1;
  }
  return counts;
}
...
export const analyzePropertyLocation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<LocationAnalysisResult> => {
    ...
    const queryText = [address, neighborhood, data.city, state, "Brasil"]
      .filter(Boolean)
      .join(", ");
    ...
    if (googleKey && googleCoordinates) {
      const [schools, health, supermarkets, parks, transit] = await Promise.all([
        ...
        googleNearbyCount(googleKey, googleCoordinates, ["transit_station", "bus_station"]).catch(
          () => 0,
        ),
      ]);
