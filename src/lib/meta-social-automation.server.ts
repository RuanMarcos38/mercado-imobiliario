...
const PAGE_SUBSCRIBED_FIELDS = ["messages", "messaging_postbacks", "feed"] as const;
...
async function subscribeMetaPageToApp(page: MetaPageConnection) {
  try {
    await graphJson(
      `https://graph.facebook.com/${graphVersion()}/${encodeURIComponent(page.pageId)}/subscribed_apps`,
      page.pageAccessToken,
      { subscribed_fields: PAGE_SUBSCRIBED_FIELDS.join(",") },
    );
    return { ok: true, fields: [...PAGE_SUBSCRIBED_FIELDS] };
  } catch (error) {
    return {
      ok: false,
      fields: [...PAGE_SUBSCRIBED_FIELDS],
      error: error instanceof Error ? error.message : "META_SUBSCRIBED_APPS_FAILED",
    };
  }
}
...
export async function registerMetaSocialConnections(input: { tenantId: string; userId: string }) {
  ...
  const rows = config.pages.map((page, index) => ({
    ...
    metadata: {
      ...registryMetadata(page, input.userId),
      webhookCallbackUrl: metaSocialWebhookCallbackUrl(),
      pageWebhookSubscription: subscriptions[index],
    },
  }));
