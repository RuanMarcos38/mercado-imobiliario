...
function LocationAnalysisPage() {
  ...
  const copyReport = async () => {
    ...
    const text = [
      ...
      `Infraestrutura: ${result.infrastructure.schools} educação, ${result.infrastructure.health} saúde, ${result.infrastructure.supermarkets} supermercados, ${result.infrastructure.parks} parques, ${result.infrastructure.transit} transporte.`,
    ...
    <div className="min-h-screen bg-[var(--mi-bg)] p-4 text-[var(--mi-text)] sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1400px]">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-[var(--mi-text-muted)]">
          Combine infraestrutura, atividade do mercado imobiliário e dados oficiais para apoiar
          uma recomendação mais qualificada ao cliente.
        </p>
          ...
          <div className="mt-6 space-y-6">
            ...
            <section className="grid gap-5 lg:grid-cols-3">
              <Panel title="Mercado observado" icon={TrendingUp}>
                ...
                <Stat label="Recentes em 90 dias" value={number(result.market.recentListings90d)} />
              ...
              <Panel title="Infraestrutura próxima" icon={Activity}>
                {result.infrastructure.available ? (
                  ...
                  <p className="text-sm leading-6 text-[var(--mi-text-muted)]">
                    Configure a chave Google Maps Platform no servidor para ativar a leitura de
                    estabelecimentos e infraestrutura em um raio de {result.infrastructure.radiusMeters / 1000} km.
                  </p>
                )}
              </Panel>
              ...
              <Panel title="Contexto municipal" icon={Users}>
                ...
                <div className="mt-4 rounded-xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3 text-xs leading-5 text-[var(--mi-text-muted)]">
                  O dado populacional contextualiza a escala urbana e não é usado isoladamente
                  como previsão de preço.
                </div>
              </Panel>
            </section>
...
function ComponentScore({ label, value, max }: { label: string; value: number; max: number }) {
  return (
    <div className="rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3">
      <p className="text-[9px] font-black uppercase tracking-[0.11em] text-[var(--mi-text-soft)]">{label}</p>
      <p className="mt-2 text-lg font-black">
        {value}<span className="text-[10px] text-[var(--mi-text-soft)]">/{max}</span>
      </p>
    </div>
  );
}
