    ...
    <div className="min-h-screen bg-[var(--mi-bg)] p-4 text-[var(--mi-text)] sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1400px]">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-[var(--mi-text-muted)]">
          Combine preços observados, infraestrutura mapeada e dados oficiais para apoiar uma recomendação mais qualificada ao cliente.
        </p>
        ...
        <section className="mt-6 rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1.5fr_1fr_1fr_120px_auto] xl:items-end">
            <Field label="Endereço ou referência">
              <Input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} placeholder="Rua, condomínio ou empreendimento" />
            </Field>
            <Field label="Bairro">
              <Input value={form.neighborhood} onChange={(event) => setForm({ ...form, neighborhood: event.target.value })} placeholder="Ex.: América" />
            </Field>
            <Field label="Cidade">
          ...
          <div key={result.analyzedAt} className="mt-6 space-y-6">
            <section className="grid gap-5 lg:grid-cols-[320px_1fr]">
              ...
              <div className="rounded-[28px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-6">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-4">
                  ...
                  <ComponentScore label="Evidência" value={result.components.marketEvidence} max={20} />
                  <ComponentScore label="Confiança" value={result.components.dataConfidence} max={15} />
                </div>
              </div>
            </section>
            ...
            <section className="grid gap-5 lg:grid-cols-3">
              <Panel title="Mercado observado" icon={TrendingUp}>
                <Stat label="Escopo usado" value={result.market.scope === "bairro" ? "Bairro" : "Município"} />
                <Stat label="Anúncios na amostra" value={number(result.market.sampleSize)} />
                <Stat label="Vistos nos últimos 90 dias" value={number(result.market.recentListings90d)} />
                ...
                {result.infrastructure.available ? (
                  <>
                    <div className="mb-3 rounded-xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3 text-xs text-[var(--mi-text-muted)]">
                      Raio de {(result.infrastructure.radiusMeters / 1000).toFixed(1)} km · fonte {result.infrastructure.provider === "google" ? "Google Maps Platform" : "OpenStreetMap"}
                    </div>
                  ...
                  <p className="text-sm leading-6 text-[var(--mi-text-muted)]">Não foi possível consultar um provedor cartográfico neste momento. Tente novamente ou informe um endereço/bairro mais específico.</p>
                )}
              </Panel>
              ...
              <Panel title="Contexto municipal" icon={Users}>
                ...
                <div className="mt-4 rounded-xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3 text-xs leading-5 text-[var(--mi-text-muted)]">O Censo 2022 é a referência censitária oficial. O dado populacional contextualiza a escala urbana e não é usado isoladamente como previsão de preço.</div>
              </Panel>
            </section>
...
function ComponentScore({ label, value, max }: { label: string; value: number; max: number }) {
  return <div className="rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3"><p className="text-[9px] font-black uppercase tracking-[0.11em] text-[var(--mi-text-soft)]">{label}</p><p className="mt-2 text-lg font-black">{value}<span className="text-[10px] text-[var(--mi-text-soft)]">/{max}</span></p></div>;
}

function Panel({ title, icon: Icon, children }: { title: string; icon: React.ComponentType<{ className?: string }>; children: React.ReactNode }) {
  return <section className="rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5"><div className="flex items-center gap-2"><Icon className="h-4 w-4 text-blue-600" /><h2 className="font-black">{title}</h2></div><div className="mt-4 space-y-3">{children}</div></section>;
}

function Stat({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4 border-b border-[var(--mi-border)] pb-3 last:border-0"><span className="text-xs text-[var(--mi-text-muted)]">{label}</span><span className="text-right text-sm font-black">{value}</span></div>;
}

function Amenity({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: number }) {
  return <div className="rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3"><Icon className="h-4 w-4 text-blue-600" /><p className="mt-2 text-xl font-black">{value}</p><p className="text-[10px] text-[var(--mi-text-soft)]">{label}</p></div>;
}
