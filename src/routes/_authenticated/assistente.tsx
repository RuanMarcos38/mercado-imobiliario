...
import { getAiAgentSettings, saveAiAgentSettings } from "@/lib/whatsapp-admin.functions";
...
function AssistantPage() {
  ...
  const [testMessage, setTestMessage] = useState(
    "Olá, estou procurando um apartamento em Joinville. Como você pode me ajudar?",
  );
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mi-text-muted)]">
              Configure o tom de atendimento, teste respostas antes de ativar e defina quando a
              conversa deve passar para uma pessoa.
            </p>
          </div>
          <StatusCard
            configured={Boolean(runtime.data?.configured)}
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_460px]">
          <section className="rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 sm:p-6">
            ...
            <div className="mt-5 space-y-4">
              ...
              <Button
                onClick={() => void save()}
                className="h-11 rounded-xl bg-blue-600 px-5 font-black text-[var(--mi-text)] hover:bg-blue-700"
              >
          ...
          <section className="rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 sm:p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--mi-text-muted)]">
              Este teste não envia mensagem para nenhum cliente. Ele apenas valida a resposta do
              assistente no servidor.
            </p>

            <div className="mt-5 rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-surface-soft)] p-4">
              <div className="flex items-start gap-3">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/5 text-[var(--mi-text-muted)]">
                ...
                <textarea
                  ...
                  className="min-h-28 flex-1 resize-none bg-transparent text-sm leading-6 text-[var(--mi-text)] outline-none placeholder:text-slate-600"
                />
              </div>
            </div>
            <Button
              ...
              className="mt-3 h-11 w-full rounded-xl bg-blue-600 font-black text-white hover:bg-blue-700"
            >
            ...
            {testReply && (
              <div className="mt-4 rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.04] p-4">
                ...
                <p className="whitespace-pre-wrap text-sm leading-6 text-[var(--mi-text)]">
                  {testReply}
                </p>
              </div>
            )}
            ...
            {!runtime.isLoading && !runtime.data?.configured && (
              <div className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/[0.04] p-4 text-sm leading-6 text-amber-100">
                O módulo está pronto, mas a inteligência artificial ainda não está conectada no
                servidor. O teste e as respostas automáticas permanecem bloqueados até a ativação.
              </div>
            )}
          </section>
        </div>
...
function StatusCard({ configured, model }: { configured: boolean; model: string | null }) {
  return (
    <div
      className={`rounded-2xl border px-4 py-3 ${configured ? "border-emerald-300/15 bg-emerald-300/[0.05]" : "border-amber-300/15 bg-amber-300/[0.04]"}`}
    >
      <p
        className={`flex items-center gap-2 text-xs font-black ${configured ? "text-emerald-200" : "text-amber-100"}`}
      >
        <ShieldCheck className="h-4 w-4" /> {configured ? "IA conectada" : "IA aguardando ativação"}
      </p>
      {model && (
        <p className="mt-1 text-[11px] text-[var(--mi-text-soft)]">Modelo do servidor: {model}</p>
      )}
    </div>
  );
}
...
function ToggleCard({
  ...
  return (
    <label
      className={`flex cursor-pointer items-start justify-between gap-4 rounded-2xl border p-4 ${warning ? "border-amber-300/10 bg-amber-300/[0.025]" : "border-[var(--mi-border)] bg-[var(--mi-surface-soft)]"}`}
    >
      <span>
        <span className="block text-sm font-bold">{title}</span>
        <span className="mt-1 block text-xs leading-5 text-[var(--mi-text-soft)]">
          {description}
        </span>
      </span>
      <input
