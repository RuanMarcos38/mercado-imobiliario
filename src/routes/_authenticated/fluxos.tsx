        ...
        <section className="mt-7 rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-black">Importar fluxo JSON</h2>
              </div>
              <p className="mt-1 max-w-3xl text-xs leading-5 text-[var(--mi-text-soft)]">
                Gere o fluxo no ChatGPT, cole ou carregue o JSON e importe diretamente no motor de
                automação do MercadoImobi. O usuário não precisa configurar nem acessar serviços
          ...
          <div className="mt-5 space-y-3">
            ...
            {jsonPreview && (
              <div className="rounded-xl border border-emerald-300/40 bg-emerald-500/[0.06] p-3 text-xs leading-5">
                <p className="font-black text-emerald-700 dark:text-emerald-300">
                  JSON válido para o motor nativo
                </p>
                <p className="mt-1">
                  Fluxo: {jsonPreview.flowName} · {jsonPreview.flowSteps} passo(s)
                </p>
              </div>
            )}

            <div className="rounded-xl border border-blue-300/30 bg-blue-500/[0.04] p-3 text-[11px] leading-5 text-[var(--mi-text-soft)]">
              <p className="font-black text-[var(--mi-text)]">
                Automação gerenciada pela plataforma
              </p>
              <p className="mt-1">
                Mensagens, espera, IA, tags, transferência para humano e webhooks são executados
                pelo próprio MercadoImobi. As integrações internas permanecem protegidas no servidor
                e não exigem URL, API Key ou login do usuário.
              </p>
            </div>
            ...
            <p className="text-[10px] leading-4 text-[var(--mi-text-soft)]">
              O fluxo é importado pausado para revisão. Depois, use o botão Ativar no cartão do
              fluxo. Nenhuma credencial existente é modificada.
            </p>
          </div>
        </section>
      </div>
