...
function AdminUsersPage() {
  ...
  const create = async () => {
    ...
    try {
      ...
      toast.error(message.includes("FORBIDDEN_ADMIN") ? "Acesso restrito ao administrador." : message || "Não foi possível criar o usuário.");
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="grid min-h-[calc(100vh-56px)] place-items-center bg-[var(--mi-bg)] p-6">
        <div className="max-w-md rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-8 text-center">
          ...
          <p className="mt-2 text-sm text-[var(--mi-text-muted)]">Somente o administrador global da plataforma pode criar e gerenciar usuários.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[var(--mi-bg)] p-4 text-[var(--mi-text)] sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-600">Administração</p>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mi-text-muted)]">
              Crie acessos, acompanhe assinaturas e suspenda contas. Cada novo usuário recebe sua própria organização para impedir mistura de dados.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <Metric label="Ativos" value={(users.data ?? []).filter((user) => user.isActive).length} />
        ...
        <section className="mt-6 overflow-hidden rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)]">
          <div className="flex flex-col gap-3 border-b border-[var(--mi-border)] p-4 sm:flex-row sm:items-center">
            <div className="relative max-w-md flex-1">
              ...
              <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar nome, e-mail ou imobiliária..." className="pl-9" />
            </div>
              ...
              <tbody>
                {filtered.map((user) => (
                  <tr key={user.id} className="border-t border-[var(--mi-border)]">
                    ...
                    <td className="px-5 py-4">
                      <p className="font-semibold">{user.tenantName || user.companyName || "Conta individual"}</p>
                      <p className="mt-1 text-xs text-[var(--mi-text-soft)]">{user.memberRole || "owner"}</p>
                    </td>
                    <td className="px-5 py-4">
                      <select
                    ...
                    <td className="px-5 py-4">
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${user.isActive ? "bg-emerald-500/10 text-emerald-700" : "bg-rose-500/10 text-rose-700"}`}>
                ...
                {!users.isLoading && filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-5 py-14 text-center text-sm text-[var(--mi-text-muted)]">Nenhum usuário encontrado.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Senha inicial">
              <div className="flex gap-2">
                ...
                <Button type="button" variant="outline" size="icon" onClick={() => setForm({ ...form, password: randomPassword() })} title="Gerar senha"><KeyRound className="h-4 w-4" /></Button>
              </div>
            </Field>
            <Field label="Tipo de usuário">
              <select value={form.userType} onChange={(e) => setForm({ ...form, userType: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                <option value="corretor">Corretor</option><option value="imobiliaria">Imobiliária</option><option value="construtora">Construtora</option><option value="proprietario">Proprietário</option><option value="cliente">Cliente</option><option value="admin">Administrador</option>
              </select>
            </Field>
            <Field label="Empresa / imobiliária"><Input value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} /></Field>
            <Field label="Assinatura inicial">
              <select value={form.subscriptionStatus} onChange={(e) => setForm({ ...form, subscriptionStatus: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                <option value="trialing">Período de teste</option><option value="active">Ativa</option><option value="unpaid">Aguardando pagamento</option>
              </select>
            </Field>
            {form.subscriptionStatus === "trialing" && <Field label="Dias de teste"><Input type="number" min={0} max={90} value={form.trialDays} onChange={(e) => setForm({ ...form, trialDays: Number(e.target.value) })} /></Field>}
          </div>
          <div className="mt-2 rounded-xl border border-blue-500/20 bg-blue-500/[0.05] p-3 text-xs leading-5 text-[var(--mi-text-muted)]">
            O cadastro cria uma organização própria para o usuário. O administrador global gerencia a conta, mas os dados operacionais permanecem separados por tenant e usuário.
          </div>
          <div className="flex justify-end gap-2 pt-2"><Button variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button onClick={() => void create()} disabled={saving || !form.fullName || !form.email || form.password.length < 8}>{saving ? "Criando..." : "Criar usuário"}</Button></div>
        </DialogContent>
      </Dialog>
    </div>
  );
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-surface)] p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--mi-text-soft)]">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>;
}
