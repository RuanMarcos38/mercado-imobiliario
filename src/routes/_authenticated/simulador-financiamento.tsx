        ...
        <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mi-text-muted)]">
              Faça uma estimativa rápida pelo sistema PRICE e, em seguida, confirme as condições
              reais no Simulador Habitacional oficial da CAIXA.
            </p>
          </div>
          <a href={CAIXA_SIMULATOR_URL} target="_blank" rel="noopener noreferrer">
            <Button className="h-11 rounded-xl bg-blue-600 px-5 text-white hover:bg-blue-700">
              <Landmark className="mr-2 h-4 w-4" /> Abrir Simulador oficial CAIXA{" "}
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.05fr]">
          <section className="rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-black">Dados da simulação</h2>
                <p className="text-xs text-[var(--mi-text-muted)]">
                  Altere os valores para recalcular automaticamente.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Prazo (meses)">
                <Input
                  ...
                  value={months}
                  onChange={(e) =>
                    setMonths(Math.min(420, Math.max(1, Number(e.target.value) || 1)))
                  }
                />
              </Field>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-4 text-xs leading-5 text-[var(--mi-text-muted)]">
              Esta calculadora é uma estimativa matemática pelo sistema PRICE. Não inclui TR,
              seguros MIP/DFI, tarifas, CET, subsídios, FGTS, avaliação de risco ou regras
              específicas da CAIXA. A proposta oficial deve ser confirmada no simulador da
              instituição.
            </div>
          </section>
          ...
          <section className="rounded-[26px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-black">Resultado estimado</h2>
                <p className="text-xs text-[var(--mi-text-muted)]">
                  Parcela fixa de principal + juros pelo PRICE.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Metric
                ...
                value={money(simulation.financed)}
                icon={<Home className="h-4 w-4" />}
              />
              <Metric
                label="Parcela PRICE estimada"
                value={money(simulation.payment)}
                emphasis
                icon={<Calculator className="h-4 w-4" />}
              />
              <Metric
                label="Total das parcelas"
                value={money(simulation.totalPaid)}
                icon={<WalletCards className="h-4 w-4" />}
              />
              <Metric
                label="Juros totais estimados"
                value={money(simulation.totalInterest)}
            ...
            <div className="mt-4 rounded-2xl border border-blue-500/20 bg-blue-500/[0.05] p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-black">Renda bruta de referência</p>
                  <p className="mt-1 text-2xl font-black text-blue-600">
                    {money(simulation.suggestedIncome)}
                  </p>
                  <p className="mt-1 text-xs leading-5 text-[var(--mi-text-muted)]">
                    Referência calculada considerando prestação equivalente a 30% da renda bruta. A
                    aprovação real depende da análise de crédito e das regras vigentes da CAIXA.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <Button variant="outline" className="h-11 w-full rounded-xl">
                Confirmar no Simulador Habitacional CAIXA <ExternalLink className="ml-2 h-4 w-4" />
              </Button>
            </a>
          </section>
        </div>
...
function Metric({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${emphasis ? "border-blue-500/25 bg-blue-500/[0.06]" : "border-[var(--mi-border)] bg-[var(--mi-bg)]"}`}
    >
      ...
      <p
        className={`mt-2 text-xl font-black ${emphasis ? "text-blue-600" : "text-[var(--mi-text)]"}`}
      >
        {value}
      </p>
    </div>
