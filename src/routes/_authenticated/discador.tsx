...
function DialerPage() {
  ...
  useEffect(() => {
    if (agentPhone.trim())
      localStorage.setItem("mercadoimobi:dialer-agent-phone", agentPhone.trim());
  }, [agentPhone]);
      ...
      <div className="mx-auto max-w-[1320px]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mi-text-muted)]">
              Ligue para leads do CRM sem copiar números. O sistema chama seu telefone primeiro e,
              depois que você atende, conecta a ligação ao cliente.
            </p>
          </div>
          <span
          ...
          >
            ...
            {status.data?.configured
              ? `Telefonia pronta ${status.data.callerNumber ?? ""}`
        ...
        {!status.isLoading && !status.data?.configured && (
          <div className="mt-6 rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-4 text-sm leading-6 text-[var(--mi-text-muted)]">
            Configure <strong>TWILIO_ACCOUNT_SID</strong>, <strong>TWILIO_AUTH_TOKEN</strong> e{" "}
            <strong>TWILIO_PHONE_NUMBER</strong> no EasyPanel. Em produção, também é possível usar{" "}
            <strong>TWILIO_API_KEY_SID</strong> e <strong>TWILIO_API_KEY_SECRET</strong>.
          </div>
        )}
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[420px_1fr]">
          <section className="rounded-[28px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 shadow-sm">
            ...
            <div className="mt-4 max-h-[560px] space-y-2 overflow-y-auto pr-1">
              {(leads.data ?? [])
                .filter((lead) => lead.client_phone)
                .map((lead) => (
                  <button
                    key={lead.id}
                    type="button"
                    onClick={() => setSelectedLeadId(lead.id)}
                    className={`w-full rounded-2xl border p-4 text-left transition ${selectedLeadId === lead.id ? "border-blue-500/40 bg-blue-500/[0.08]" : "border-[var(--mi-border)] bg-[var(--mi-surface-soft)] hover:border-blue-500/25"}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block text-xs text-[var(--mi-text-soft)]">
                          {lead.client_phone}
                        </span>
          ...
          <section className="rounded-[28px] border border-[var(--mi-border)] bg-[var(--mi-surface)] p-5 shadow-sm sm:p-7">
            <div className="mx-auto max-w-md">
              <div className="text-center">
                <span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-blue-500/10 text-blue-600">
                  <PhoneCall className="h-7 w-7" />
                </span>
                <h2 className="mt-3 text-xl font-black">Nova chamada</h2>
                <p className="mt-1 text-xs text-[var(--mi-text-soft)]">
                  Chamadas comerciais com origem identificada pelo número de telefonia configurado.
                </p>
              </div>

              <label className="mt-6 block">
                <span className="mb-1.5 block text-xs font-black text-[var(--mi-text-muted)]">
                  Seu telefone para receber a ponte
                </span>
                <input
                  value={agentPhone}
                  onChange={(event) => setAgentPhone(event.target.value)}
                  placeholder="47 99999-9999"
                  className="h-12 w-full rounded-xl border border-[var(--mi-border)] bg-[var(--mi-surface-soft)] px-4 text-center text-lg font-black tracking-wide outline-none focus:border-blue-500"
                />
              </label>
              <label className="mt-4 block">
                ...
                <input
                  ...
                  className="h-14 w-full rounded-xl border border-[var(--mi-border)] bg-[var(--mi-surface-soft)] px-4 text-center text-2xl font-black tracking-wide outline-none focus:border-blue-500"
                />
              </label>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                {KEYS.map((key) => (
                  <button
                    ...
                    className="h-14 rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-surface-soft)] text-lg font-black transition hover:border-blue-500/30 hover:bg-blue-500/[0.05]"
                  >
                    {key}
                  </button>
                ))}
              ...
              <Button
                onClick={() => void call()}
                disabled={calling || !status.data?.configured}
                className="mt-5 h-14 w-full rounded-2xl bg-emerald-600 text-base font-black text-white hover:bg-emerald-700"
              >
                <Phone className="mr-2 h-5 w-5" />{" "}
                {calling ? "Iniciando chamada..." : "Ligar para o cliente"}
              </Button>

              {lastCall && (
                <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.05] p-4 text-sm">
                  <p className="font-black text-emerald-700">Chamada solicitada</p>
                  <p className="mt-1 text-[var(--mi-text-muted)]">
                    Cliente: {lastCall.customerPhone}
                  </p>
                  <p className="text-[var(--mi-text-muted)]">Status inicial: {lastCall.status}</p>
                </div>
              )}
            </div>
          </section>
