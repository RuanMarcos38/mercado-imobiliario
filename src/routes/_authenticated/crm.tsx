...
function CrmPage() {
  ...
  const byStage = useMemo(() => {
    ...
    for (const lead of leads.data ?? []) {
      const stage = CRM_STAGES.includes(lead.status as CrmStage)
        ? (lead.status as CrmStage)
        : "novo";
        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mi-text-muted)]">
              Organize clientes, imóveis, propostas, documentação e análise CAIXA em um único
              Kanban. O quadro exibe somente as oportunidades do usuário autenticado.
            </p>
          </div>
          ...
          {CRM_STAGES.map((stage) => (
            ...
            >
              <div className="flex items-center justify-between px-1 py-2">
                <h2 className="text-xs font-black uppercase tracking-[0.11em]">
                  {CRM_STAGE_LABELS[stage]}
                </h2>
                <span className="grid h-6 min-w-6 place-items-center rounded-full bg-[var(--mi-bg)] px-1.5 text-[10px] font-black text-[var(--mi-text-muted)]">
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      >
                        ...
                        <p className="mt-1 line-clamp-2 text-xs leading-5 text-[var(--mi-text-muted)]">
                          {lead.ai_qualification_notes || "Sem imóvel vinculado"}
                        </p>
                      </button>
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-[var(--mi-border)] pt-3">
                      <button
                        onClick={() => setSelected(lead)}
                        className="flex items-center gap-1.5 text-[10px] font-black text-blue-600"
                      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Imóvel / oportunidade">
              <Input
                value={form.propertyReference}
                onChange={(e) => setForm({ ...form, propertyReference: e.target.value })}
                placeholder="Empreendimento, endereço ou código"
              />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Observações">
                <Textarea
                  ...
                  placeholder="Perfil, renda, entrada, prazo, próximos passos..."
                />
              </Field>
            </div>
          </div>
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => void create()} disabled={saving || form.name.trim().length < 2}>
...
function LeadDossier({
  ...
  lead: CrmLead;
  onStageChanged: () => Promise<unknown>;
}) {
  ...
  const stageFn = useServerFn(updateCrmLeadStage);
  const docs = useQuery({
    queryKey: ["cca-documents", lead.id],
    queryFn: () => listDocsFn({ data: { leadId: lead.id } }),
  });
  const [category, setCategory] =
    useState<(typeof CCA_DOCUMENT_CATEGORIES)[number]>("identificacao_comprador");
    ...
    <>
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2">
          <UserRound className="h-5 w-5 text-blue-600" /> {lead.client_name}
        </DialogTitle>
      </DialogHeader>
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Info
          ...
          value={
            CRM_STAGE_LABELS[
              (CRM_STAGES.includes(lead.status as CrmStage) ? lead.status : "novo") as CrmStage
            ]
          }
        />
      </div>
      <div className="rounded-2xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-4">
        ...
        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[var(--mi-text-muted)]">
          {lead.ai_qualification_notes || "Sem observações."}
        </p>
      </div>
      ...
      <section className="rounded-[22px] border border-[var(--mi-border)] p-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h3 className="flex items-center gap-2 font-black">
              <FileText className="h-4 w-4 text-blue-600" /> Dossiê documental / CCA
            </h3>
            <p className="mt-1 text-xs text-[var(--mi-text-muted)]">
              PDF, JPG, PNG ou WebP. Máximo de 12 MB por arquivo.
            </p>
          </div>
          <Button
            variant="outline"
            onClick={() => void send()}
            disabled={submitting || (docs.data?.length ?? 0) === 0}
          >
            <Send className="mr-2 h-4 w-4" /> {submitting ? "Enviando..." : "Enviar ao CCA"}
          </Button>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-[260px_1fr]">
          ...
          >
            {CCA_DOCUMENT_CATEGORIES.map((value) => (
              <option key={value} value={value}>
                {CCA_DOCUMENT_LABELS[value]}
              </option>
            ))}
          </select>
        ...
        <div className="mt-4 space-y-2">
          {(docs.data ?? []).map((document) => (
            <div
              key={document.path}
              className="flex items-center gap-3 rounded-xl border border-[var(--mi-border)] bg-[var(--mi-bg)] p-3"
            >
              <FileText className="h-4 w-4 shrink-0 text-blue-600" />
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold">{document.name}</p>
                <p className="text-[10px] text-[var(--mi-text-soft)]">
                  {CCA_DOCUMENT_LABELS[document.category as keyof typeof CCA_DOCUMENT_LABELS] ??
                    document.category}
                </p>
              </div>
              {document.signedUrl && (
              ...
              <button
                onClick={async () => {
                  await removeDocFn({ data: { leadId: lead.id, path: document.path } });
                  await docs.refetch();
                }}
                aria-label="Remover"
              >
          ...
          {!docs.isLoading && (docs.data?.length ?? 0) === 0 && (
            <p className="rounded-xl bg-[var(--mi-bg)] p-4 text-center text-xs text-[var(--mi-text-muted)]">
              Nenhum documento anexado.
            </p>
          )}
        </div>
        <p className="mt-4 text-[11px] leading-5 text-[var(--mi-text-soft)]">
          O envio direto é ativado somente quando o endpoint oficial ou contratado do seu CCA
          estiver configurado no servidor. Sem essa credencial, o MercadoImobi mantém o dossiê
          privado e pronto para envio.
        </p>
      </section>
    </>
  );
...
function Badge({
  ...
  return (
    <span className="flex max-w-full items-center gap-1 rounded-full bg-[var(--mi-surface)] px-2 py-1 text-[9px] font-bold text-[var(--mi-text-muted)]">
